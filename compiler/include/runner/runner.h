...
class Runner {
   ...
   bool loadLLVM(std::string fileName);
   bool loadString(std::string input);
   bool optimize(runtime::Database& db);
   bool dhap_compile(runtime::Database& db);
   bool lower();
   bool lowerSplitModules(std::string subq_id_str);
   bool lowerToLLVM();
   bool lowerSplitModulesToLLVM();
   
   bool lowerToLLVMExceptSCF();
   bool lowerSCFToLLVMInnerPar();
   void dump();
   ...
   ~Runner();
   static void printTable(std::shared_ptr<arrow::Table>* ptr);
