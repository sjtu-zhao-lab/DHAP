...
class ColumnSet {
   ...
   // Keep a column from current set if its name is in `other`
   ColumnSet intersectOnColName(MLIRContext* context, const ColumnSet& other) const {
      ColumnSet result;
      auto& attributeManager = context->getLoadedDialect<mlir::relalg::RelAlgDialect>()->getColumnManager();
      for (const auto* x : attributes) {
         std::string x_name = attributeManager.getColName(x);
         for (const auto* y : other.attributes) {
            std::string y_name = attributeManager.getColName(y);
            if (x_name == y_name) {
               result.insert(x);
               break;
            }
   ...
   void remove(const mlir::relalg::Column* col) {
      attributes.erase(col);
   }
