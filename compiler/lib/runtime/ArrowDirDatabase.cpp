...
void storeTable(std::string file, std::shared_ptr<arrow::Table> table) {
   ...
   assert(batchWriter->WriteTable(*table, 1 << 10).ok());
   // std::cout << "Finish write table" << std::endl;
   assert(batchWriter->Close().ok());
   // std::cout << "Finish close writer" << std::endl;
   assert(inputFile->Close().ok());
   // std::cout << "Finish close file" << std::endl;
}
...
std::unique_ptr<Database> ArrowDirDatabase::load(std::string directory, bool sample_only) {
   std::string json;
   ...
   for (const auto& p : std::filesystem::directory_iterator(directory)) {
      auto path = p.path();
      // std::cout << "SEARCH FOR " << path.filename() << std::endl;
      if (path.extension().string() == ".arrow" && !sample_only) {
      ...
      if (path.extension().string() == ".sample" && path.stem().string().ends_with(".arrow")) {
         ...
         auto sample = loadSample(path.string());
         database->samples[tablename] = sample;
         if (sample_only)
            database->tables[tablename] = arrow::Table::FromRecordBatches({sample}).ValueOrDie();
      }
   ...
   if (sample_only)  assert(has_sample);
...
std::shared_ptr<arrow::RecordBatch> createSample(std::shared_ptr<arrow::Table> table) {
   ...
   arrow::compute::TakeOptions opt;
   opt.boundscheck = false;
   auto res = arrow::compute::CallFunction("take", args, &opt).ValueOrDie();
   // std::cout << "Finish sample take function" << std::endl;
...
ArrowDirDatabase::~ArrowDirDatabase() {
   if (writeback) {
      ...
      for (auto t : tables) {
         if (t.second->num_rows() > 0)
            storeTable(directory + "/" + t.first + ".arrow", t.second);
      }
      for (auto s : samples) {
         if (s.second->num_rows() > 0)
