...
std::unique_ptr<Database> Database::loadFromDirSample(std::string directory) {
   return ArrowDirDatabase::load(directory, true);
}
...
void Database::copyFromIntoTable(runtime::VarLen32 tableName, runtime::VarLen32 fileName, runtime::VarLen32 delimiter, runtime::VarLen32 escape) {
   std::cout << tableName.str() << std::endl;
   std::cout << fileName.str() << std::endl;
   std::cout << delimiter.str() << std::endl;
   std::cout << escape.str() << std::endl;
   ...
   std::cout << "Got a table with " << table->num_rows()
             << " rows and"  << " batches" << std::endl;
