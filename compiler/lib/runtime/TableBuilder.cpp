...
void runtime::TableBuilder::addBinary(bool isValid, runtime::VarLen32 string) {
   ...
   if (!isValid) {
      ...
      std::cout << string.getLen() << std::endl;
      std::cout << *(string.getPtr()) << std::endl;
...
std::vector<std::shared_ptr<arrow::RecordBatch>> runtime::TableBuilder::get_batches() {
   flushBatch();
   return batches;
}
void runtime::TableBuilder::mergeBatches(TableBuilder* tb) {
   for (auto tb_batch : tb->get_batches()) {
      batches.push_back(tb_batch);
   }
}
