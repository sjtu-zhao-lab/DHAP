...
class ForIteratorIterationImpl : public mlir::dsa::CollectionIterationImpl {
   ...
   virtual std::vector<Value> implementLoop(mlir::Location loc, mlir::ValueRange iterArgs, Value flag, mlir::TypeConverter& typeConverter, ConversionPatternRewriter& builder, ModuleOp parentModule, std::function<std::vector<Value>(std::function<Value(OpBuilder&)>, ValueRange, OpBuilder)> bodyBuilder) override {
      if (this->inner_par) {
         return implementLoopSimpleParallel(loc, iterArgs, typeConverter, builder, bodyBuilder);
      }
      if (flag) {
   ...
   std::vector<Value> implementLoopSimpleParallel(mlir::Location loc, const ValueRange& iterArgs, TypeConverter& typeConverter, ConversionPatternRewriter& builder, std::function<std::vector<Value>(std::function<Value(OpBuilder&)>, ValueRange, OpBuilder)> bodyBuilder) {
      assert(iterArgs.size()==0);
      auto insertionPoint = builder.saveInsertionPoint();
      iterator->setTypeConverter(&typeConverter);
      iterator->init(builder);
      iterator->setLoc(loc);
      auto ParaOp = builder.create<scf::ParallelOp>(loc, iterator->lower(builder), iterator->upper(builder), iterator->step(builder), llvm::None);
      Operation* terminator = ParaOp.getBody()->getTerminator();
      builder.setInsertionPointToStart(ParaOp.getBody());
      std::vector<Value> bodyArguments = {};

      assert(ParaOp.getInductionVars().size()==1);
      // bodyArguments.insert(bodyArguments.end(), ParaOp.getRegionIterArgs().begin(), forOp.getRegionIterArgs().end());
      Value element;
      auto results = bodyBuilder([&](mlir::OpBuilder& b) { return element = iterator->getElement(b, ParaOp.getInductionVars()[0]); }, bodyArguments, builder);
      builder.restoreInsertionPoint(insertionPoint);
      return std::vector<Value>(ParaOp.getResults().begin(), ParaOp.getResults().end());
   }
